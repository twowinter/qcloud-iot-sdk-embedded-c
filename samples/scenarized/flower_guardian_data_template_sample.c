...
typedef struct _ProductDataDefine {
    TYPE_DEF_TEMPLATE_BOOL m_pump_switch; 
    TYPE_DEF_TEMPLATE_FLOAT m_soil_moisture;
...
static void _init_data_template(void)
{
	...
	sg_ProductData.m_pump_switch = 0;
    sg_DataTemplate[0].data_property.key  = "pump_switch";
    sg_DataTemplate[0].data_property.data = &sg_ProductData.m_pump_switch;
    sg_DataTemplate[0].data_property.type = TYPE_TEMPLATE_BOOL;

	sg_ProductData.m_soil_moisture = 10;
    sg_DataTemplate[1].data_property.key  = "soil_moisture";
    sg_DataTemplate[1].data_property.data = &sg_ProductData.m_soil_moisture;
...
static void deal_down_stream_user_logic(ProductDataDefine *flower_guardian)
{
	if(flower_guardian->m_pump_switch){
        /* 灯光开启式，按照控制参数展示 */
		HAL_Printf("[  pump is on  ]|[brightness:%d]|[%s]\n", flower_guardian->m_soil_moisture, flower_guardian->m_name);
	}else{
		/* 灯光关闭展示 */
		HAL_Printf("[  pump is off ]|[brightness:%d]|[%s]\n", flower_guardian->m_soil_moisture, flower_guardian->m_name);	
	}
